<template>
    <div class="container">
        ...
        <Task3_DelteModal :videoId="currentVideoId" v-if="showModalFlag" @close-modal="showModalFlag = false" @delete-video="deleteVideo"/>
    </div>
</template>

<script>
import axios from 'axios';
import Task3_DelteModal from './Task3_DeleteModal.vue';

export default {
    name: "Task5_VideoOverview",
    components: {
        Task3_DelteModal
    },
    data() {
    ...
    mounted() {
        ...
        axios.get('/task/get-all-task', {
            params: {
                task: 30,  // Assuming task number 3 is for videos
            }
        })
            .then(response => {
