<template>
    <div class="container">
        ...
        <Task5_DelteModal :videoId="currentVideoId" v-if="showModalFlag" @close-modal="showModalFlag = false" @delete-video="deleteVideo"/>
    </div>
</template>

<script>
import axios from 'axios';
import Task5_DelteModal from './Task5_DeleteModal.vue';
...
export default {
    ...
    mounted() {
        ...
        axios.get('/task/get-all-task', {
            params: {
                task: 50,  // Assuming task number 3 is for videos
            }
        })
            .then(response => {
